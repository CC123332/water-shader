...
//     float colorRamp(float t){
//         // Key stops
//         float black1 = 0.545454;
//         float white  = 0.777273;
//         float black2 = 0.931818;
//         float color = 0.0;
...
//         // Time factor cycles from 0 → 300, normalized by /60.0
//         float timeFactor = mod(t, 300.0) / 60.0;
//         vec3 FMB_noise = fbm(color_input + timeFactor);

//         //Linear Light
//         vec3 linear = linearLight(FMB_noise, formatted_UV, color_input.r);

//         //Voronoi
//         float worleyOutput1 = voronoiSmoothF1(linear + formatted_UV, vec3(1.0), 0., 1.0);
//         float worleyOutput2 = voronoiSmoothF1(linear + formatted_UV, vec3(1.0), .5, 1.0);
//         float subtract = worleyOutput1 - worleyOutput2;


//         //Surface Tone
//         float Surface_tone = subtract * 2.;

//         gl_FragColor = vec4(vec3(subtract), 1.0);
//     }
// `;
...
export const blurFrag = /* glsl */`
    ...
    uniform float uRadius;  // runtime radius (<= MAX_RADIUS)
    uniform float uSigma;   // Gaussian sigma; larger => slower decay

    // Compile-time max radius to keep the loop unrolled by the compiler
    ...
    void main() {
        ...
        float w0 = 1.0;
        if (uSigma > 0.0) {
            // normalized so fi=0 -> weight 1
            w0 = 1.0;
        }
        acc  += texture2D(uTex, vUv).r * w0;
        wsum += w0;
        ...
        for (int i = 1; i <= MAX_RADIUS; ++i) {
            ...
            // Gaussian weight with tunable decay
            float w = exp(-0.5 * (fi * fi) / (uSigma * uSigma));

            vec2 ofs = uDir * uTexel * fi;
            acc  += texture2D(uTex, vUv + ofs).r * w;
            acc  += texture2D(uTex, vUv - ofs).r * w;
            wsum += 2.0 * w;
        }

        float blurred = acc / max(wsum, 1e-6);
...
export const blurMatX = new THREE.ShaderMaterial({
    uniforms: {
        uTex:    { value: null },
        uTexel:  { value: new THREE.Vector2(1/FBO_SIZE, 1/FBO_SIZE) },
        uDir:    { value: new THREE.Vector2(1, 0) }, // horizontal; change to (0,1) for vertical
        uRadius: { value: 40 },   // larger reach
        uSigma:  { value: 16 },   // slower decay
    },
...
export const blurMatY = new THREE.ShaderMaterial({
    uniforms: {
        uTex:    { value: null },
        uTexel:  { value: new THREE.Vector2(1/FBO_SIZE, 1/FBO_SIZE) },
        uDir:    { value: new THREE.Vector2(0, 1) }, // horizontal; change to (0,1) for vertical
        uRadius: { value: 40 },   // larger reach
        uSigma:  { value: 16 },   // slower decay
    },
    vertexShader: blurVert,
